class ClienteMesa {
    constructor(nome,telefone,qunatidade,visitas,chaveCliente) {
      this._nome = nome;
  ...
  const carrinhoRef = database.ref("carrinhoBalcao");
  ...
  carrinhoRef.once("value")
    .then(snapshot => {
      if (snapshot.exists()) {
        console.log("O nó carrinhoBalcao existe no Firebase.");
        ...
        console.log("O nó carrinhoBalcao não existe no Firebase.");
      }
    })
    .catch(error => {
      console.error("Ocorreu um erro ao verificar o nó carrinhoBalcao:", error);
    });
